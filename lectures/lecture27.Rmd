...
* HW 5: Hw solutions posted
* HW 6: Due today.  Tomorrow at noon solutions will be posted
* Midterm make-up and all HWs graded by Tuesday evening
* TODAY: Review the practice problems and discuss all topics we've covered in class
...
* By the model we have $e_i/\sigma \sim N(0,1)$ and therefore $e_i^2/ \sigma^2 \sim \chi^2_1$ and so $E[e_i^2/\sigma^2] = 1/\sigma^2$.  So this means we replace $E E$ with $\text{diag}(\sigma^2)$ and we recover the model-based variance formula.
...
$$
\frac{\sum_h (N_h/N)/(r_h/r) n_h \bar y_h}{\sum_h (N_h/N)/(r_h/r) n_h } = `r round(sum(psw/response_rate*n*meany)/sum(psw/response_rate*n),2)`
$$
...
$$
\frac{\sum_h (N_h/N)/(r_h/r) n_h \bar y_h}{\sum_h (N_h/N)/(r_h/r) n_h } = `r round(sum(psw_2/response_rate*n*meany)/sum(psw_2/response_rate*n),2)`
$$

## Problem 5

Consider respondent driven sampling in a setting where the network consists of two fully connected components of 100 people and the two components are connected by a single edge (i.e., there is one unit in each component that are linked and that's it).  What is stationary distribution?  Given a seed that is not the unit connected to the other component, what is the distribution of time until switching components?  Suppose that the components split completely so one component has binary 0 outcome 0 and the other has binary outcome 1.  Then the true mean is $50\%$.   Simulate out chains of lengths 10, 50, 100, and 1000.  Compute estimates and confidence intervals.  Use standard ratio variance formula to estimate the variance of the ratio estimator.  Use the Volz-Heckathorn estimator.  Discuss coverage of the confidence interval.

## Problem 5:

* $d_i = 100$ for all $i$ except the bridge which has $d_i = 101$.  Stationary distribution is approximately uniform with weights $100/(198*100 + 2*101) = `r round(100/(198*100 + 2*101), 4)`$ and $100/(198*100 + 2*101) = `r round(101/(198*100 + 2*101), 4)`$
* Waiting time to get to brige is Geometric with $p=1/100$, and then there is a chance to cross or not cross with probabilty $1/101$.  So let $H_l \sim Geom(1/100)$ and $C_l \sim Bern(1/101)$ be iid for $l=1,\ldots$ then the holding time until crossing is given by
$$
\begin{aligned}
Z &= H_1 C_1 + (1-C_1)(H_1 + H_2 C_2) + \cdots \\
&= \sum_{l=1}^\infty \left[ \prod_{j=1}^{l-1} (1-C_j) \right] 
\left( \sum_{j=1}^{l-1} H_j + H_l C_l \right)
\end{aligned}
$$

## Problem 5:

* The expectation of $H_l$ is $100$ and of $C_l$ is $1/101$.
* So we have

$$
\begin{aligned}
&E[ H_1 ] E[C_1] + (1-E[C_1]) E[H_1] \\
+ &(1-E[C_1]) ( E[C_2] E[H_2] + (1-E[C_2]) E[H_3] +\ldots) \\
= & E[H_1] + (1-E[C_1]) E[H_2] + (1-E[C_1]) (1-E[C_2]) E[H_3] + \ldots \\ 
= & 100 \left( 1 + (1-1/101) + (1-1/101)^2 + \ldots \right) \\
= & 100 \sum_{l=0}^\infty (100/101)^l \\
= & 100 \times \frac{1}{1-100/101} = 100 \times 101 = 10100
\end{aligned}
$$
...
## Topic 20: Respondent driven sampling
...
* Variance calculation via _Volz-Heckathorn_:
$$
