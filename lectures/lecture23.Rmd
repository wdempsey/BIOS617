...
* HW 5: Due today (HW solutions posted tomorrow)
* HW 6: Posted end of day today (last hw!)
* HW 4 grades and solutions posted
* Grading policy

```{r, out.width = "300px", fig.align='center'}
include_graphics("./figs/um-grade-policy.png") # place holder
```
* TODAY: Non-sampling error + more Big Data paradox discussion 
...
* __Stochastic__: assume that each element of the population has a probability of response $P_i$
  ...
  + $\bar P = \frac{1}{N} \sum_{i=1}^N P_i$K
...
&= \sum_h W_h \bar Y_{Rh} - \sum_h W_h (1-R_h) \bar Y_{Rh} + \sum_h (1-R_h) \bar Y_{Mh} \\
...
&= R^{-1} \sum_{h} R_h W_h \bar Y_{Rh} - \left[ \sum_h W_h \bar Y_{Rh} - \sum_h W_h (1-R_h) (\bar Y_{Rh} - \bar Y_{Mh})  \right]  \\
