...
n_pew = sum(table(data$ideo))
f_pew = n_pew/189500000
p_moderate = sum(data$ideo_numeric == 0)
p_liberal = sum(data$ideo_numeric == -1)
p_conservative = sum(data$ideo_numeric == 1)
frac_pew = c(p_liberal, p_moderate, p_conservative)/n_pew

data %>%  ggplot(aes(ideo_numeric)) +
  geom_bar(aes(y = (..count..)/sum(..count..)), data=data, fill = "blue", alpha = 0.2) +
  xlab("Ideological buckets") +
  ylab("Frequency") +
  labs(color = "Ideology type") +
  ggtitle("Ideology response from Pew 2008")
```

## Linear regression of ideology

```{r pew2, echo = FALSE}
lm(ideo_numeric~sex+age+marital, data = data) %>%
  summary() %>% 
  xtable() %>% 
  kable(digits=3)
```
...
## Design-based inference: Pew 2008
* Suppose we want to know number of liberal, moderate, and conservative voters
* U.S. Population in 2008 was 304.1 million; Percent of eligible voters was 62.3\% (N=189.5 million)
* The sample has `r n_pew` out of the population; f = `r f_pew`
* Population has `r round(frac_pew,3)` fraction of individuals who identify as \emph{liberal}, \emph{moderate}, and \emph{conservative} 
* Variance of $v(p)$ are `r (1-f_pew) * frac_pew *(1-frac_pew)/(n_pew-1)`
* Sample std. errors are `r round(sqrt((1-f_pew) * frac_pew *(1-frac_pew)/(n_pew-1)),3)` 

```{r pew_subset, echo = FALSE}
pew_subset = subset(data, survey == "sept08forpoli")
n_pew2 = sum(table(pew_subset$ideo))
p_moderate2 = sum(pew_subset$ideo_numeric == 0)
p_liberal2 = sum(pew_subset$ideo_numeric == -1)
p_conservative2 = sum(pew_subset$ideo_numeric == 1)
frac_pew2 = c(p_liberal2, p_moderate2, p_conservative2)/n_pew2
f_pew2 = n_pew2/189500000
```

## Design-based inference: Pew 2008 (single survey)
* Suppose we want to know number of liberal, moderate, and conservative voters
* U.S. Population in 2008 was 304.1 million; Percent of eligible voters was 62.3\% (N=189.5 million)
* The sample has `r n_pew2` out of the population; f = `r f_pew2`
* Population has `r round(frac_pew2,3)` fraction of individuals who identify as \emph{liberal}, \emph{moderate}, and \emph{conservative} 
* Variance of $v(p)$ are `r (1-f_pew2) * frac_pew2 *(1-frac_pew2)/(n_pew2-1)`
* Sample std. errors are `r round(sqrt((1-f_pew2) * frac_pew2 *(1-frac_pew2)/(n_pew2-1)),3)` 
