...
&= \frac{1}{f^2} \sum_{i=1}^N  C(I_i, I_j)  X_i X_j Y_i \\
...
* If we had data on the entire populatoin, we could solve
...
* This is just a population total; thus general unbiased estimator of the population score equatoin is given by
...
$$
X^\top W Y = X^\top W X b_w \Rightarrow b_w = (X^\top W X)
$$
...
$$
\begin{aligned}
V(b_w) = V(U_w^{-1} ( U_w (b_w))) &= \left( \frac{\partial g}{\partial {\bf u}} \mid_{{\bf u} = {\bf U}} \right)^\top V( {\bf u}) \left( \frac{\partial g}{\partial {\bf u}} \mid_{{\bf u} = {\bf U}} \right) \\
