...
$$
\underbrace{\rho_{I, Y}}_{\text{Data Quality}} \times \underbrace{\sqrt{ \frac{1-f}{f} }}_{\text{Data Quantity}}\times \underbrace{\sigma_Y}_{\text{Problem Difficulty}} = D_I \times D_O \times D_U
$$
...
* Can we estimate $E_{I} \left[ \rho_{I, Y}^2 \right]$ for non-probabilistic data?
* For binary data, we can show Hoeffding-Frechet bounds

$$
- \min \left\{ \sqrt{\frac{D_O}{O_Y}}, \sqrt{\frac{O_Y}{D_O}} \right\} \leq \rho_{I, Y} \leq \min \left\{ \sqrt{O_Y D_O}, \frac{1}{\sqrt{O_Y D_O}} \right\}
$$

where $O_Y = p_Y/ (1-p_Y)$ is the odds ratio. Can you think about instances where the lower and upper bound are achieved?

*  Combine prior work to show that,
$$
\frac{\bar y_n - \bar Y}{\sqrt{V_{SRS} (\bar y_n)}} = \sqrt{N-1} \rho_{I,Y}
$$
What is the design effect in terms of $N$ and $D_I$?