...
celldata = subset(data, sample=="cell")
landlinedata = subset(data, sample=="landline")

celldata %>%  ggplot(aes(ideo_numeric)) +
  geom_bar(aes(y = (..count..)/sum(..count..)), data=subset(data,sample == 'cell'), fill = "blue", alpha = 0.2) +
  geom_bar(aes(y = (..count..)/sum(..count..)), data=subset(data,sample == 'landline'), fill = "green", alpha = 0.2) +
...
```{r pew2, echo = FALSE, eval = TRUE}
cellstrata1 = celldata$age>=18 & celldata$age <= 24
cellstrata2 = celldata$age>=25 & celldata$age <= 44
cellstrata3 = celldata$age >= 45

llstrata1 = landlinedata$age>=18 & landlinedata$age < 24
llstrata2 = landlinedata$age>=25 & landlinedata$age < 44
llstrata3 = landlinedata$age >= 45

P1 = 9.6/(1-0.258)
P2 = 30.2/(1-0.258)
P3 = (22+12.4)/(1-0.258)

cellest = mean(celldata$ideo_numeric[cellstrata1])*P1/100 + mean(celldata$ideo_numeric[cellstrata2])*P2/100 + mean(celldata$ideo_numeric[cellstrata3])*P3/100
llest = mean(landlinedata$ideo_numeric[llstrata1])*P1/100 + mean(landlinedata$ideo_numeric[llstrata2])*P2/100 + mean(landlinedata$ideo_numeric[llstrata3])*P3/100
```

* The mean age in all the data is `r round(mean(data$age),2)`, in just the cell surveys is `r round(mean(celldata$age),2)`, and in just the landlines is `r round(mean(data$age[data$sample=="landline"]),2)`.
* Split cell data into strata of 18-24 ($P_1 = `r round(P1,3)`$\%), 25-44 ($P_2 = `r round(P2,3)`$\%), and 45+ ($P_3 = `r round(P3,3)`$\%)
* The mean ideology is `r round(mean(celldata$ideo_numeric[cellstrata1]),3)`, `r round(mean(celldata$ideo_numeric[cellstrata2]),3)`, and `r round(mean(celldata$ideo_numeric[cellstrata3]),3)` respectively in __cell__ data.
* The mean ideology is `r round(mean(landlinedata$ideo_numeric[llstrata1]),3)`, `r round(mean(landlinedata$ideo_numeric[llstrata2]),3)` and `r round(mean(landlinedata$ideo_numeric[llstrata3]),3)` respectively in __landline__ data.
* PS estimates are `r round(cellest,2)` for cell only data and `r round(llest,2)` for landline only data
* Overall estimate was `r round(mean(data$ideo_numeric),2)`
...
\begin{aligned}
&E ( \sum_{i=1}^k V (\bar y_i) )  + \frac{k}{K} \sum_{i=1}^K \bar Y_i^2 \\
=& \sum_{i=1}^k \left( 1 - \frac{m}{M} \right) \frac{E(S_i^2)}{m}  + \frac{k}{K} \sum_{i=1}^K \bar Y_i^2 \\
...
* Find more recent Pew poll data (or make some assumptions in the Pew 2008 example)
* What if we were more interested in the result of the election than the current poll?
* How would you convert the current poll results into an estimate of the election outcome? 